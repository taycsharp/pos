...
const createSingleProduct = async (req, res) => {
  if (req.query.query === "deletemany") {
    ...
    try {
      ...
      createdProduct.thumbnail_image_url = await getObjectSignedUrl(imageName);
      // stock product's account transaction create
      await prisma.transaction.create({
        data: {
          date: new Date(),
  ...
  if (req.query.query === "all") {
    try {
      ...
      for (let product of allProduct) {
        if (product.thumbnail_image) {
          ...
          product.thumbnail_image_url = await getObjectSignedUrl(thumbnail_image);
        }
      }
      res.json(allProduct);
    } catch (error) {
    ...
    try {
      ...
      for (let product of allProduct) {
        if (product.thumbnail_image) {
          //product.thumbnail_image_url = `${HOST}:${PORT}/v1/product-image/${product.thumbnail_image}`;
          product.thumbnail_image_url = await getObjectSignedUrl(product.thumbnail_image);
        }
      }
      res.json(allProduct);
    } catch (error) {
...
const getSingleProduct = async (req, res) => {
  try {
    ...
    if (singleProduct && singleProduct.thumbnail_image) {
      ...
      singleProduct.thumbnail_image_url = await getObjectSignedUrl(singleProduct.thumbnail_image);
    }
...
const deleteSingleProduct = async (req, res) => {
  try {
    ...
    // TODO: implement delete all image from aws s3 related to this product
    if (deletedProduct && deletedProduct.thumbnail_image) {
      await deleteFile(deletedProduct.thumbnail_image);
